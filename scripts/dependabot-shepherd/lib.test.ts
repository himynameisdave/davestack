...
describe('bumpLevel', () => {
  ...
  it('handles v-prefixes, bare majors (actions), and prerelease tags', () => {
    ...
    expect(bumpLevel('7.0.2-beta.1', '7.0.2')).toBe('patch');
...
describe('parseUpdates', () => {
  it('parses a single-dependency PR from its body', () => {
    expect(parseUpdates('Bump zod from 4.4.3 to 4.5.0', SINGLE_BODY)).toEqual([
...
describe('maxLevel / levelAllowed / parseAutoMergeLevel', () => {
  ...
  it('only lets unknown bumps through when the ceiling is major', () => {
    ...
    expect(levelAllowed('unknown', 'major')).toBe(true);
  });

  it('defaults an unrecognised ceiling to minor', () => {
    expect(parseAutoMergeLevel('MAJOR')).toBe('major');
    expect(parseAutoMergeLevel('yolo')).toBe('minor');
    expect(parseAutoMergeLevel(undefined)).toBe('minor');
  });
});

describe('extractCompareRefs', () => {
  it('dedupes the compare links Dependabot embeds', () => {
    expect(extractCompareRefs(GROUP_BODY)).toEqual([
      { owner: 'prisma', repo: 'prisma', base: '7.10.0', head: '7.11.0' },
    ]);
    expect(extractCompareRefs(SINGLE_BODY)).toEqual([
      { owner: 'colinhacks', repo: 'zod', base: 'v4.4.3', head: 'v4.5.0' },
    ]);
  });
});
...
describe('summarizeChecks', () => {
  it('is green only when every check run and status context passed', () => {
    ...
    expect(summary.passing).toEqual(['CI / lint', 'CI / e2e', 'deploy/preview']);
  });
...
describe('parseVerdict', () => {
  it('accepts a bare or fenced JSON object and normalises the lists', () => {
    const raw = JSON.stringify({ ...VERDICT, findings: ['x', '', 3] });
    expect(parseVerdict(raw)?.findings).toEqual(['x']);
...
describe('applyPolicy', () => {
  ...
  it('merges only when every gate is open', () => {
    expect(applyPolicy(base)).toEqual({ merge: true, reason: VERDICT.summary });
  });
