...
function cleanVersion(raw: string): string {
  return raw.trim().replace(/^[v=]/u, '').replace(/[.,;:)\]]+$/u, '');
}

/** Leading dotted-numeric part of a version (`7.1.0-beta.2+build` → `[7, 1, 0]`). */
function numericParts(version: string): number[] | null {
  const core = cleanVersion(version).split(/[-+]/u)[0] ?? '';
...
/**
 * Semver level of a bump, in Dependabot's own terms (`version-update:semver-*`): purely numeric,
 * so `0.7.0 → 0.8.0` is "minor" here even though caret ranges treat it as breaking — the review
 * prompt tells Codex to weigh 0.x bumps accordingly. Non-numeric versions (git SHAs) → `unknown`.
 */
export function bumpLevel(from: string, to: string): SemverLevel {
  const a = numericParts(from);
  const b = numericParts(to);
  if (a === null || b === null) {
    return 'unknown';
  }
  if ((a[0] ?? 0) !== (b[0] ?? 0)) {
...
/** `unknown` bumps (unparseable versions, or nothing parsed) only pass when the ceiling is `major`. */
...
export function parseAutoMergeLevel(raw: string | undefined): AutoMergeLevel {
...
/** Parses Codex's final message. Tolerates a ```json fence; returns null for anything off-schema. */
...
export function renderTemplate(template: string, vars: Readonly<Record<string, string>>): string {
  return template.replaceAll(/\{\{(\w+)\}\}/gu, (whole, key: string) => vars[key] ?? whole);
}
...
export function renderReport(results: readonly Result[], opts: Readonly<{ dryRun: boolean }>): string {
  ...
  for (const r of [...results].sort((a, b) => a.pr - b.pr)) {
    lines.push(
...
export function renderVerdictComment(
  ...
  if (verdict.checks_performed.length > 0) {
    lines.push('', '<details><summary>Checks performed</summary>', '');
    lines.push(...verdict.checks_performed.map((c) => `- ${c}`));
    lines.push('', '</details>');
  }
  lines.push(
    '',
