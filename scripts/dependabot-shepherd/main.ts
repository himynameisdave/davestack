#!/usr/bin/env bun
/**
 * Dependabot shepherd — the orchestration CLI behind `.github/workflows/dependabot-shepherd.yml`.
 *
 * Codex only *judges*; everything with side effects is deterministic and lives here:
 *
 *   discover          list open Dependabot PRs → `prs` / `count` job outputs
 *   sync    --pr N    rebase via `@dependabot rebase|recreate` if behind/conflicting, wait for CI
 *   prepare --pr N    gather the review packet (PR body, diff, upstream compare patches) + prompt
 *   review  --pr N    run `codex exec` (read-only sandbox, schema-constrained JSON verdict)
 *   decide  --pr N    apply the merge policy → `gh pr merge` or a PR comment explaining why not
 *   report  --dir D   render every PR's result into the job summary
 *
 * Configuration is env-only (see the workflow's `env:` block); `GH_TOKEN` + `GH_REPO` are what `gh`
 * needs. Runs locally too — `GH_REPO=owner/repo DRY_RUN=1 bun scripts/dependabot-shepherd/main.ts
 * discover` — for anything up to `prepare`; `review` needs a Codex login, `decide` needs write access.
 */
...
const SHEPHERD_DIR = resolve(envStr('SHEPHERD_DIR', join(tmpdir(), 'dependabot-shepherd')));
const PR_CHECKOUT = resolve(envStr('PR_CHECKOUT', join(process.cwd(), 'pr')));
const PROMPT_FILE = resolve(envStr('PROMPT_FILE', '.github/dependabot-shepherd/prompt.md'));
const SCHEMA_FILE = resolve(envStr('SCHEMA_FILE', '.github/dependabot-shepherd/verdict.schema.json'));
...
const MERGE_METHOD = ['squash', 'merge', 'rebase'].includes(envStr('MERGE_METHOD', 'squash'))
  ? envStr('MERGE_METHOD', 'squash')
  : 'squash';
...
const CODEX_MODEL = envStr('CODEX_MODEL', '');
const CODEX_REASONING_EFFORT = envStr('CODEX_REASONING_EFFORT', '');
const CODEX_TIMEOUT_MIN = envMinutes('CODEX_TIMEOUT_MINUTES', 20);
const CODEX_HOME = resolve(envStr('CODEX_HOME', join(SHEPHERD_DIR, 'codex-home')));
...
function warn(msg: string): void {
  console.log(`::warning::${msg.replaceAll(/\r?\n/gu, ' ')}`);
}
...
function discover(): void {
  ...
  const numbers = eligible.map((pr) => pr.number).sort((a, b) => a - b);
...
async function sync(n: number): Promise<void> {
  let pr = viewPr(n);
  const updates = parseUpdates(pr.title, pr.body);
  ...
  const state: State = { pr, updates, level: maxLevel(updates), checks };
...
function fetchUpstreamDiff(ref: Readonly<CompareRef>): string {
  ...
  try {
    ...
    for (const file of cmp.files ?? []) {
      ...
      parts.push(`# ${file.status} +${file.additions} -${file.deletions}\n`);
...
function codexEnv(apiKey: string): Record<string, string> {
  // A curated environment: Codex's own sandbox strips *KEY*/*TOKEN* vars from tool calls, and the
  // read-only sandbox has no network — but there's no reason to hand it the runner's env at all.
  const base: Record<string, string> = {
    PATH: envStr('PATH', '/usr/local/bin:/usr/bin:/bin'),
    HOME: envStr('HOME', tmpdir()),
    TMPDIR: envStr('TMPDIR', tmpdir()),
    CODEX_HOME,
    CI: 'true',
    NO_COLOR: '1',
    TERM: 'dumb',
  ...
  if (apiKey !== '') {
    base.CODEX_API_KEY = apiKey;
  }
  return base;
}

function authMode(apiKey: string, authPath: string): 'api-key' | 'chatgpt' | 'none' {
  if (apiKey !== '') {
    return 'api-key';
  }
  return existsSync(authPath) ? 'chatgpt' : 'none';
}

function syncAuthBack(before: string): void {
  const authPath = join(CODEX_HOME, 'auth.json');
  if (sha256(authPath) === before) {
    return;
  }
  const token = envStr('CODEX_AUTH_SYNC_TOKEN', '');
  if (token === '') {
    warn(
      'Codex refreshed its ChatGPT tokens during this run but CODEX_AUTH_SYNC_TOKEN is not set, so the '
        + 'CODEX_AUTH_JSON secret was not updated. Refresh tokens rotate — the stored secret will go stale.',
    );
    return;
  }
  log('auth.json changed — writing the refreshed tokens back to the CODEX_AUTH_JSON secret');
  gh(['secret', 'set', 'CODEX_AUTH_JSON', '--repo', REPO], {
    input: readFileSync(authPath, 'utf8'),
    env: { GH_TOKEN: token, GITHUB_TOKEN: token },
  });
}

function review(n: number): void {
  const state = loadState(n);
  const dir = contextDir(n);
  const promptPath = join(dir, 'prompt.md');
  const verdictPath = join(prDir(n), 'verdict.json');
  if (!existsSync(promptPath)) {
    fail(`no prompt for PR #${n} — run \`prepare --pr ${n}\` first`);
  }
  if (!existsSync(PR_CHECKOUT)) {
    fail(`PR checkout not found at ${PR_CHECKOUT} (set PR_CHECKOUT)`);
  }

  const apiKey = envStr('OPENAI_API_KEY', '');
  const authPath = join(CODEX_HOME, 'auth.json');
  const mode = authMode(apiKey, authPath);
  if (mode === 'none') {
    fail('no Codex credentials: set the OPENAI_API_KEY secret or CODEX_AUTH_JSON (see the workflow header)');
  }
  log(`Codex auth: ${mode}${CODEX_MODEL === '' ? '' : ` · model ${CODEX_MODEL}`}`);
  const codexEnvironment = codexEnv(apiKey);
  const authBefore = sha256(authPath);

  const status = spawnSync('codex', ['login', 'status'], { encoding: 'utf8', env: codexEnvironment });
  log(`codex login status → ${(status.stdout + status.stderr).trim() || `exit ${status.status ?? '?'}`}`);

  const args = ['exec', '--sandbox', 'read-only', '--skip-git-repo-check', '-C', PR_CHECKOUT];
  args.push('--output-schema', SCHEMA_FILE, '-o', verdictPath);
  if (CODEX_MODEL !== '') {
  ...
  const run = spawnSync('codex', args, {
    ...
    timeout: CODEX_TIMEOUT_MIN * 60 * 1000,
  ...
  if (run.error !== undefined) {
    reviewError =
      'code' in run.error && run.error.code === 'ETIMEDOUT'
        ? `Codex timed out after ${CODEX_TIMEOUT_MIN} min`
        : `could not run codex: ${run.error.message}`;
  } else if (run.status !== 0) {
    reviewError = `codex exec exited ${run.status ?? '?'}`;
  }
  const raw = existsSync(verdictPath) ? readFileSync(verdictPath, 'utf8') : run.stdout;
  const verdict = parseVerdict(raw);
  if (verdict === null && reviewError === '') {
    reviewError = 'Codex returned a message that does not match the verdict schema';
  }
  if (reviewError !== '') {
    warn(`#${n}: ${reviewError}`);
    writeFileSync(join(prDir(n), 'codex-stdout.txt'), run.stdout);
  } else {
    log(`verdict: ${verdict?.decision} · risk ${verdict?.risk} · confidence ${verdict?.confidence}`);
  }
  writeJson(statePath(n), { ...state, verdict, reviewError });

  if (mode === 'chatgpt') {
    syncAuthBack(authBefore);
  }
}
...
function mergePr(pr: Readonly<PrView>, verdict: Readonly<Verdict>): string {
  const args = ['pr', 'merge', String(pr.number), '--repo', REPO, `--${MERGE_METHOD}`];
  if (MERGE_METHOD !== 'rebase') {
    const trailer = RUN_URL === '' ? '' : `: ${RUN_URL}`;
    args.push('--subject', `${pr.title} (#${pr.number})`);
    args.push('--body', `${verdict.summary}\n\nAuto-merged by the Dependabot shepherd${trailer}`);
  }
  gh(args);
  // Best effort: GitHub's "automatically delete head branches" setting may already have done this.
  gh(['api', '-X', 'DELETE', `repos/${REPO}/git/refs/heads/${pr.headRefName}`], { allowFailure: true });
...
function decide(n: number): void {
  ...
  if (fresh.headRefOid !== state.pr.headRefOid) {
    const reason = 'head changed during review — retry next run';
    writeResult(resultFrom(fresh, state.updates, 'skipped', reason, verdict));
    return;
  }

  const policy =
    state.reviewError !== undefined && state.reviewError !== ''
      ? { merge: false, reason: state.reviewError }
      : applyPolicy({
          verdict,
          level: state.level,
          ceiling: CEILING,
  ...
  try {
    ...
    writeResult(resultFrom(fresh, state.updates, 'skipped', `merge failed: ${message}`, verdict));
  }
}
...
try {
  switch (command) {
    ...
    case 'report':
      ...
      break;
    default:
      fail(`unknown command "${command}" — one of: discover, sync, prepare, review, decide, report`);
  }
} catch (error) {
  const message = error instanceof Error ? error.message : String(error);
